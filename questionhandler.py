...
def q7(compliance, questionNumber):
    ...
    questionLabel = ctk.CTkLabel(root, text="Do you use encryption and access control technologies when processing client data?", font=questionFont)
    questionLabel.pack(pady=25)
    
    selectedOption = tk.IntVar()

    answer1checkbox = ctk.CTkRadioButton(root, text="Yes, we use both encryption and access control technologies when processing client data", font=normalFont, variable=selectedOption, value=1)
    answer1checkbox.pack(pady=15)

    answer2checkbox = ctk.CTkRadioButton(root, text="We use encryption but not access control when processing client data", font=normalFont, variable=selectedOption, value=2)
    answer2checkbox.pack(pady=15)

    answer3checkbox = ctk.CTkRadioButton(root, text="We use access control but not encryption when processing client data", font=normalFont, variable=selectedOption , value=3)
    answer3checkbox.pack(pady=15)
